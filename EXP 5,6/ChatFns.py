...
def LoadMyEntry(ChatLog, EntryText):
    if EntryText != '':
        ...
        if ChatLog.index('end') != None:
            ...
            ChatLog.insert(END, "Rohan : " + EntryText)
            ChatLog.tag_add("Rohan ", LineNumber, LineNumber+0.4)
            ChatLog.tag_config("Rohan " , foreground="#eb3434", font=("Arial", 18, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)
    ...
    if EntryText != '':
        ...
        if ChatLog.index('end') != None:
            try:
                ...
                pass
            ChatLog.insert(END, "Loki: " + EntryText)
            ChatLog.tag_add("Loki ", LineNumber, LineNumber+0.6)
            ChatLog.tag_config("Loki ", foreground="#34ebc0", font=("Arial", 18, "bold"))
            ChatLog.config(state=DISABLED)
            ChatLog.yview(END)
